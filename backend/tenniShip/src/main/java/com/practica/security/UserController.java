...
public class UserController {
	...
	public String newUser(Model model, @ModelAttribute("personUser") User user, @RequestParam String passwordCheck,
		...
		if (canContinue) {
			...
			imgService.saveImage("registered", teamNew.getName(), imageFile.get(0));
			for (int i = 1; i < 6; i++) {
				imgService.saveImage("registered", teamNew.getName() + String.format("Player%d", i), imageFile.get(i));
			}
